...
contract FullSystemTest is BaseTest {
  ...
  function test_FullLifecycle_HappyPath() public {
    ...
    // Use getClaimableDividends to avoid off-by-one from sequential integer divisions
    uint256 expectedMonth2 = distributor.getClaimableDividends(projectId, investor1);
    uint256 bal1Month2 = usdc.balanceOf(investor1);
    vm.prank(investor1);
    distributor.claimDividends(projectId);
    uint256 claimedMonth2 = usdc.balanceOf(investor1) - bal1Month2;
    assertEq(claimedMonth2, expectedMonth2);
  ...
  function test_MultiMonthAccumulation() public {
    ...
    // Contract accumulates dividendPerShare then divides once; manual per-month division
    // can differ by a few wei due to integer truncation — allow tolerance of 1 per month.
    assertApproxEqAbs(claimable, totalDividendForInvestor1, 5);

    uint256 balBefore = usdc.balanceOf(investor1);
    vm.prank(investor1);
    distributor.claimDividends(projectId);
    assertEq(usdc.balanceOf(investor1) - balBefore, claimable);
  }
  ...
  function test_MultipleProjects() public {
    ...
    vm.prank(hostA);
    reputation.mintSbt(hostA);
    vm.prank(hostB);
    reputation.mintSbt(hostB);

    weatherOracle.setRainyDays(projectId1, 0);
    weatherOracle.setRainyDays(projectId2, 0);
