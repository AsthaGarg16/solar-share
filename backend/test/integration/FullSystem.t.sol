...
contract FullSystemTest is BaseTest {
    ...
    function test_FullLifecycle_HappyPath() public {
        ...
        vm.prank(host);
        uint256 projectId = solarProject.initializeProject("Solar Demo", TARGET_AMOUNT, TERM_MONTHS, TOTAL_SHARES);
    ...
    function test_MultipleProjects() public {
        ...
        vm.prank(hostA);
        uint256 projectId1 = solarProject.initializeProject("Solar Demo", TARGET_AMOUNT, TERM_MONTHS, TOTAL_SHARES);

        vm.prank(hostB);
        uint256 projectId2 = solarProject.initializeProject("Solar Demo", TARGET_AMOUNT, TERM_MONTHS, TOTAL_SHARES);
    ...
    function testParametricPayoutBypass() public {
        ...
        vm.prank(host);
        uint256 proposalId = dao.submitProposal(projectId, "Emergency", 500 * 10 ** 6, payable(vendorAddr));
