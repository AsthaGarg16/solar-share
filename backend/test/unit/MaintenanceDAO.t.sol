...
contract MaintenanceDAOTest is BaseTest {
    ...
    function test_RequestWeatherVerification() public {
        vm.prank(investor1);
        uint256 proposalId = dao.submitProposal(projectId, "Storm Damage", PROPOSAL_AMOUNT, payable(vendor));
    ...
    function test_NoBypass_WhenWeatherIsGood() public {
        vm.prank(investor1);
        uint256 proposalId = dao.submitProposal(projectId, "Normal Wear", PROPOSAL_AMOUNT, payable(vendor));
