...
contract LoanManager is Ownable, ILoanManager {
  ...
  /// @notice Initialize loan — only callable by SolarProject (triggered via withdrawFunds)
  function initializeLoan(
    uint256 projectId,
    uint256 monthlyPayment,
    uint256 termMonths
  ) external override {
    if (msg.sender != address(solarProject)) revert NotSolarProject();
  ...
  function payMonthlyInstallment(uint256 projectId) external override {
    ...
    if (loan.totalPaid >= loan.totalOwed) {
      ...
      // Automatically sweep unused maintenance reserve and insurance pool
      // back to investors the moment the loan is fully repaid.
      revenueDistributor.settleCompletedProject(projectId);
    }
  }
  ...
  function calculateEquitySplit(
    uint256 projectId
  ) external view override returns (uint256 hostPercent, uint256 investorPercent) {
    LoanDetails storage loan = projectLoans[projectId];
  ...
  /// @notice Returns true if all loan payments have been made
  function isLoanCompleted(uint256 projectId) external view override returns (bool) {
    return projectLoans[projectId].isCompleted;
  }
}
