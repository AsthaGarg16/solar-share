...
//////////////////////// Import the IoT Oracle interface ////////////////////////
...
contract LoanManager is Ownable, ILoanManager {
    ...
    // State variable for the IoT Killswitch Oracle
    IIoTSolarOracle public iotOracle;
    ...
    // Safely link the deployed IoT Oracle without breaking existing tests
    function setIotOracle(address _oracle) external onlyOwner {
        iotOracle = IIoTSolarOracle(_oracle);
    }

    /// @notice Initialize loan for a funded project
    function initializeLoan(uint256 projectId, uint256 monthlyPayment, uint256 termMonths) external override {
        ...
        console.log("5", loan.initialized);
    ...
    function declareDefault(uint256 projectId) external override {
        ...
        // 1. Slash host reputation (Approach 2)
        hostReputation.slashScore(host, DEFAULT_PENALTY);

        // 2. Trigger the IoT Killswitch
        if (address(iotOracle) != address(0)) {
            // IT IS NOW DYNAMIC! 
            string memory realHardwareId = solarProject.getHardwareId(projectId);
            iotOracle.triggerKillswitch(projectId, realHardwareId);
        }
