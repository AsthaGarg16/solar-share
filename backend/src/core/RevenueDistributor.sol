...
contract RevenueDistributor is AccessControl, IRevenueDistributor {
    ...
    event WaterfallExecuted(uint256 indexed projectId, uint256 dividend, uint256 maintenance, uint256 insurance);
    ...
    function getClaimableDividends(uint256 projectId, address investor) external view override returns (uint256) {
    ...
    function getMaintenanceReserve(uint256 projectId) external view override returns (uint256) { return projectRevenue[projectId].maintenanceReserve; }

    function withdrawInsurance(uint256 projectId, uint256 amount) external override onlyRole(DEFAULT_ADMIN_ROLE) {
      RevenuePool storage pool = projectRevenue[projectId];
