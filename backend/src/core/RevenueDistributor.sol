...
contract RevenueDistributor is AccessControl, IRevenueDistributor {
    ...
    event PriceFeedUpdated(address indexed newDataFeed); // ADDED
    ...
    ISolarRevenueOracle public gridOracle;
    
    // Chainlink Price Feed interface (ORACLE)
    ...
    function setGridOracle(address _gridOracle) external onlyRole(DEFAULT_ADMIN_ROLE) {
        gridOracle = ISolarRevenueOracle(_gridOracle);
    }
    
    // Safely link the Chainlink Price Feed without breaking existing tests (ORACLE)
    function setDataFeed(address _dataFeed) external onlyRole(DEFAULT_ADMIN_ROLE) {
        dataFeed = AggregatorV3Interface(_dataFeed);
        emit PriceFeedUpdated(_dataFeed);
    }

    // Read the real-time exchange rate (ORACLE)
    function getLatestPrice() public view returns (int256) {
        if (address(dataFeed) == address(0)) revert PriceFeedNotSet();
