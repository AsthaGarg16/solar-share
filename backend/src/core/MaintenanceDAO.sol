...
// Interface for the Weather Oracle
...
contract MaintenanceDAO is Ownable {
    /*//////////////////////////////////////////////////////////////
                                 ERRORS
    //////////////////////////////////////////////////////////////*/
    ...
    // State variable for the Weather Oracle
    ...
    // Ownable inheritance so we can securely set the oracle later
    constructor(address _solarProject, address _revenueDistributor, address _usdc) Ownable(msg.sender) {
    ...
    // Setter function to link the deployed Weather Oracle
    ...
    function executeProposal(uint256 proposalId) external {
        ...
        if (passed) {
            ...
            uint256 rainyDays = 0;
            if (address(weatherOracle) != address(0)) {
                rainyDays = weatherOracle.getRainyDays(proposal.projectId);
            }

            if (rainyDays > 15) {
                // path A: Bad weather (Insurance)
                usedInsurance = true;
                revenueDistributor.withdrawInsurance(proposal.projectId, proposal.amount);
                usdc.transfer(proposal.vendor, proposal.amount);
            } else {
                // path B: Hardware failure (Maintenance)
        ...
        emit ProposalExecuted(proposalId, passed, proposal.yesVotes, proposal.noVotes, usedInsurance);
    }
