...
interface ILoanManager {
  ...
  function calculateEquitySplit(
    uint256 projectId
  ) external view returns (uint256 hostPercent, uint256 investorPercent);
  ...
  // --- NEW: Oracle & Automation Setters ---

  /// @notice Connects the IoT Oracle so LoanManager can trigger the killswitch
  function setIoTOracle(address _iotOracle) external;

  /// @notice Connects the Automation contract so LoanManager can auto-add projects to monitoring
