...
contract WeatherOracle is FunctionsClient, Ownable, IOracles {
    ...
    function setMaintenanceDAO(address _dao) external onlyOwner { maintenanceDAO = _dao; }
    ...
    function triggerHardwareLock(uint256) external pure override { revert(); }
    function unlockHardware(uint256) external pure override { revert(); }
}