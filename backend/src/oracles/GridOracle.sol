...
contract GridOracle is FunctionsClient, Ownable, IOracles {
    ...
    constructor(address router, bytes32 _donId, uint64 _subscriptionId, address _usdc, address _distributor) 
    ...
    function getProduction(uint256 projectId, uint256 monthIndex) external view override returns (uint256) {
    ...
    function triggerHardwareLock(uint256) external pure override { revert("IoT only"); }
    function unlockHardware(uint256) external pure override { revert("IoT only"); }
    
    function setLoanManager(address _loanManager) external onlyOwner { loanManager = _loanManager; }
    function setJavascriptSource(string calldata _sourceCode) external onlyOwner { sourceCode = _sourceCode; }
}