...
export default function ProjectDetailPage() {
  ...
  const fundingPct =
    project.targetAmount > 0n ? Number((project.amountRaised * 100n) / project.targetAmount) : 0;
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1 font-mono">Host: {project.host}</p>
        </div>
          ...
          <div className="bg-slate-800/60 border border-slate-700/50 rounded-2xl p-6">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              {[
                ...
                {
                  label: "Start Date",
                  value: project.isFunded ? formatDate(project.startDate) : "Not started",
                },
              ].map((item) => (
                <div
                  key={item.label}
            ...
            <div className="bg-slate-800/60 border border-slate-700/50 rounded-2xl p-6">
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                {[
                  ...
                  {
                    label: "Current Month",
                    value: `${loanData.currentMonth} / ${loanData.termMonths}`,
                  },
                  { label: "Next Payment Due", value: formatDate(loanData.nextPaymentDue) },
                  {
                    label: "Status",
                    value: loanData.isCompleted
                      ? "Completed"
                  ...
                  return (
                    <div className="mt-4 p-4 bg-slate-900/60 rounded-xl">
                      <p className="text-slate-400 text-sm mb-2">
                        Current Equity Split (Amortization)
                      </p>
                      <div className="flex gap-4">
                        <div>
                          <p className="text-2xl font-bold text-green-400">
                            {investorPct.toString()}%
                          </p>
              ...
              return (
                <div className="bg-slate-800/60 border border-slate-700/50 rounded-2xl p-6">
                  ...
                  <div className="grid sm:grid-cols-3 gap-4">
                    {[
                      {
                        label: "Dividend Pool (93%)",
                        value: `$${formatUSDC((rev as any)?.[1] || 0n)}`,
                        color: "text-green-400",
                      },
                      {
                        label: "Maintenance (5%)",
                        value: `$${formatUSDC((rev as any)?.[2] || 0n)}`,
                        color: "text-yellow-400",
                      },
                      {
                        label: "Insurance (2%)",
                        value: `$${formatUSDC((rev as any)?.[3] || 0n)}`,
                        color: "text-blue-400",
                      },
                    ].map((item) => (
        ...
        <div className="space-y-4">
          ...
          {loanData?.isCompleted && (
            <div className="bg-emerald-900/20 border border-emerald-500/20 rounded-xl p-4 text-sm text-emerald-400">
              Loan fully repaid. All reserves have been automatically returned to investors as
              dividends.
            </div>
          )}
